...
import {youTubeSearch} from '../apiclient.js';
...
COMMANDS.set('add', async (bot, username, msg) => {
  ...
  if (vid.type === 'yt' && bot.youtubeApiKey) {
    ...
    if (vid.id == null) {
      const data = await youTubeSearch(msg, bot.youtubeApiKey);
      const vidResult = {
        id: data.id,
        type: 'yt',
      };
      bot.addVideo(null, null, null, pos, vidResult);
      bot.sendChatMsg(
          `[b]Added result:[/] [b][pink]${data.title}[/] to the queue Yes https://youtu.be/${data.id}`);
    } else {
      bot.addVideo(null, null, null, pos, vid);
    }
  }
...
COMMANDS.set('addemote', async (bot, username, msg, isPm) => {
  ...
  await sleep(50);
  if (isPm) {
    bot.sendPm(username, `Added emote ${emoteToAdd}`);
  } else {
...
COMMANDS.set('addrandom', async (bot, username, msg) => {
  if (!(await bot.checkPermission(username, Rank.MOD, 'R'))) {
    bot.sendChatMsg(`Awkward ${username} does not have permission to addrandom.`);
    return;
  }
...
COMMANDS.set('ar', async (bot, username, msg) => {
  if (!(await bot.checkPermission(username, Rank.MOD, 'R'))) {
    bot.sendChatMsg(`Awkward ${username} does not have permission to addrandom.`);
    return;
  }
  ...
  const num = parseInt(msg.split(' ')[0], 10);
  if (isNaN(num)) {
    bot.sendChatMsg(`Couldn't parse count. Ex: @ar 10`);
    return;
  }
...
COMMANDS.set('blacklist', async (bot, username, msg, isPm) => {
  if (!(await bot.checkPermission(username, Rank.MOD, null))) {
  ...
  if (isPm) {
    bot.blacklistVideo();
    bot.sendPm(username, `Blacklisted current video.`);
    return
  }  
...
COMMANDS.set('unblacklist', async (bot, username, msg) =>  {
  if ((!(await bot.checkPermission(username, Rank.OWNER, null))) && (username !== 'ItMePeachy')) {
    bot.sendChatMsg(`https://cdn.discordapp.com/attachments/984312367162732554/984350106373480468/linaxWut.png [pink]${username} what do you think you're doing?`);
    return;
  }
  bot.sendChatMsg(`Unblacklisted current video.`);
  bot.unblacklistVideo();
});


COMMANDS.set('', (bot, username, msg) => {
...
COMMANDS.set('choose', (bot, username, msg) => {
  if (!msg) {
    bot.sendChatMsg(`AWKWARD ${username} you need to give me choices and seperate them with a comma`)
    return;
  }
  ...
  bot.sendChatMsg(`[b]${username},[/] ${choice}`);
});
...
COMMANDS.set('deletevideos', async (bot, username, msg) => {
  if (username != 'ItMePeachy') {
    bot.sendChatMsg(`${username} does not have permission to deletevideos. WeirdCat2`);
    return;
  }
...
const /** @type {Handler} */ emoteCountHandler = async (bot, username, msg) => {
  ...
  if ((msg.split(' ')[0].startsWith('/')) || (msg.split(' ')[0].startsWith('!'))) {
    bot.sendChatMsg(`Nice try forsenCD`);
    return;
  }
...
COMMANDS.set('help', (bot, username, msg) => {
  bot.sendChatMsg('https://docs.google.com/document/d/1Vct3wORMTGovmzIK2MvRdi43vbWFNrIVtduyu1a6tO0/edit?usp=sharing');
});
...
COMMANDS.set('lastseen', async (bot, username, msg) => {
  const target = msg.split(' ')[0] !== '' ? msg.split(' ')[0] : null;

  if (!target) {
    bot.sendChatMsg(`Failed to provide username Pepega Clap`);
    return;
  }
  if (!bot.lastseenLimiter.tryRemoveTokens(1)) {
    return bot.sendPm(username, '$lastseen is on cooldown');
  }
  const lastSeen = await bot.db.getLastSeen(target);
  if (!lastSeen) {
    bot.sendChatMsg('User was not found BadMan');
    return;
  }

  const quotedMsg = filterMsg(lastSeen.msg);
  const time = new Date(lastSeen.timestamp);
    ...
    if (words[w].endsWith('do')) {
      for (let w = 0; w < words.length; w++) {
        if (words[w].startsWith('poof')) {
          words[w] = 'p00f';
          bot.sendChatMsg(
              `[aqua]${lastSeen.username}[/] was last seen [b]${timestamp}[/] ` +
              `%^^%Msg: ${words.join(' ')}`);
          return;
        }
      }
  ...
  bot.sendChatMsg(
      `[aqua]${lastSeen.username}[/] was last seen ` +
      `[b]${timestamp}[/] %^^%Msg: ${quotedMsg}`);
});

COMMANDS.set('nuke', async (bot, username, msg) => {
  if (!msg) {
    return;
  }
  if (!(await bot.checkPermission(username, Rank.MOD))) {
  ...
  const words = msg.split(' ');
  let nukeSecs = parseDuration(words[0], /** format= */ 'sec');
  if (isNaN(nukeSecs) || nukeSecs < 0) {
    bot.sendChatMsg('Failed to parse nuke length.');
    return;
  }
  const timeoutSecs = parseDuration(words[1], /** format= */ 'sec');
  if (isNaN(timeoutSecs) || timeoutSecs < 0) {
    bot.sendChatMsg('Failed to parse timeout length.');
    return;
  }
  const nukePhrase = msg.split(' ').slice(2).join(' ');
  if (!nukePhrase) {
    bot.sendChatMsg('Failed to parse nuke phrase.');
  }
  bot.sendPm(username, `Nuked target phrase for ${nukeSecs}s %^^%Timeout: ${timeoutSecs}s`);
  const nukeEnd = getCurrentUnixTimestamp() + nukeSecs;
  const lastMsgTS = [];
  for (nukeSecs * 2; nukeSecs > 0; nukeSecs--) {
    for (let a = 0; a < 20; a++) {
      const lastMsgs = await bot.db.nukeRecentMsgs(a);
      const timeouts = await bot.db.getAllTimeouts();
      const end = getCurrentUnixTimestamp() + timeoutSecs;
      const user = lastMsgs.username;
      const timestamp = lastMsgs.timestamp;
      if ((lastMsgs.msg.toLowerCase().includes(nukePhrase.toLowerCase())) &&
          (!(timeouts.includes(user))) && (!(lastMsgTS.includes(timestamp)))) {
        bot.sendChatMsg(`/mute ${user} Matched nuke phrase`);
        bot.sendChatMsg(`/me Timed out ${user} for ${timeoutSecs}s`);
        await bot.db.setUserTimeout(user, end);
      }
      lastMsgTS.push(timestamp);
    }
    await sleep(500);
    const currentTime = getCurrentUnixTimestamp();
    if (currentTime > nukeEnd) {
      return;
    }
  }
});
...
/*COMMANDS.set('mute', async (bot, username, msg) => {
  if (!(await bot.checkPermission(username, Rank.OWNER))) {
...
COMMANDS.set('poof', async (bot, username, msg) => {
  ...
  bot.sendChatMsg(`/me Poofed ${username} for (RNG) ${lengthDesc} ppPoof`);
...
COMMANDS.set('spam', async (bot, username, msg) => {
  ...
  if (username === 'MrDestructoidCyDJ') {
    bot.sendChatMsg(
        `treatyourselfnow [pink][b]${username}`);
    return;
  }
  if ((!(await bot.checkPermission(username, Rank.OWNER, null))) && (username !== 'ItMePeachy')) {
    bot.sendChatMsg(`Awkward [b][pink]${username}[/] you're not allowed to do that`);
    return;
  }
...
COMMANDS.set('tempban', async (bot, username, msg, isPm) => {
  if (!msg) {
    return;
  }
  if (!(await bot.checkPermission(username, Rank.MOD, 'I'))) {
  ...
  if (isNaN(lengthSecs) || lengthSecs < 0) {
    bot.sendChatMsg('Failed to parse tempban length. Example: @tempban ItMePeachy 10m');
    return;
  }
...
COMMANDS.set('timeout', async (bot, username, msg, isPm) => {
  if (!msg) {
    return;
  }
  if (!(await bot.checkPermission(username, Rank.MOD, null))) {
...
COMMANDS.set('mute', async (bot, username, msg, isPm) => {
  if (!msg) {
    return;
  }
  if (!(await bot.checkPermission(username, Rank.MOD, null))) {
...
COMMANDS.set('title', async (bot, username, msg) => {
  ...
  if (cmdCheck.startsWith('/')) {
    bot.sendChatMsg(`forsenCD Nice try. %^^%Title: ${channel.title}`);
    return;
  } else if (cmdCheck.startsWith('@')) {
    bot.sendChatMsg(`forsenCD Nice try. %^^%Title: ${channel.title}`);
    return;
  }
...
COMMANDS.set('unmute', async (bot, username, msg) => {
  if (!(await bot.checkPermission(username, Rank.ADMIN, 'M'))) {
...
COMMANDS.set('pcount', async (bot, username, msg, isPm) => {
  if (username != 'IP0G') {
    return;
  }
  if (!msg) {
  ...
  if (msg.includes('do poof')) {
    return bot.sendChatMsg(`Nice try forsenCD`);
  }

  const count = await bot.db.getPhraseCount(msg);

  if (isPm) {
    bot.sendPm(username, `${msg} has been used ${count} times.`);
  } else {
    bot.sendChatMsg(`${msg} has been used ${count} times.`);
  }
});
