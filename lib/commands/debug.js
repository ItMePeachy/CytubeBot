...
COMMANDS.set('debugchat', async (bot, username, msg) => {
  if (username !== 'ItMePeachy') {
    bot.sendChatMsg('Only peachy can run that peepoSit');
    return;
  }
...
COMMANDS.set('restartcount', async (bot, username, msg) => {
  if (!(await bot.checkPermission(username, Rank.MOD, 'K'))) {
    bot.sendChatMsg(`${username} does not have permission use this command. FeelsWeirdMan`);
    return;
  }
  bot.sendChatMsg(`Restart count: ${await bot.readTimes()}`);
});
