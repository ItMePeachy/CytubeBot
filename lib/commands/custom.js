...
import {getCurrentUnixTimestamp, sleep, parseMediaLink} from '../utils.js';
...
COMMANDS.set('gn', (bot, username, msg) => {
  bot.sendChatMsg(`ManyPats2 [pink]goodnight ${username}, sweet dreams`);
});
...
COMMANDS.set('tuck', (bot, username, msg) => {
  ...
  if (target === '') {
    bot.sendChatMsg(
        `${username}, who do you want to tuck in? nekoConfused ` +
        'Example: @tuck PeachyBot');
  }

  bot.sendChatMsg(`TuckAHomie [pink][b]${username}[/] tucks [b]${target}[/] into bed. sweet dreams [b]${target}.[/]`);
});

COMMANDS.set('mc', (bot, username, msg) => {
  bot.sendChatMsg(
      `catHop [b][pink]${username}, We have a Modded Minecraft Server thats open` +
...
COMMANDS.set('ship', async (bot, username, msg) => {
  ...
  if (first === '') {
    bot.sendChatMsg('Invalid syntax. Examples: $ship ItMePeachy PeachyBot / $ship ItMePeachy');
    return;
  }
...
COMMANDS.set('jam', async (bot, username, msg) => {
  if (!(await bot.checkPermission(username, Rank.MOD))) {
    bot.sendChatMsg(`${username} does not have permission to use $jam. Erm `);
    return;
  }
  const words = msg.split(' ');
  const waitTime = parseDuration(words[0], /** format= */ 'sec');
  ...
  const waitTimeMs = waitTime * 1000;
  if (waitTimeMs < 1000) {
    bot.sendChatMsg(`Duration between messages is too short Erm `);
    return;
  }
...
COMMANDS.set('balls', async (bot, username, msg) => {
  bot.sendChatMsg('⠀');
  await sleep(150)
  bot.sendChatMsg(`https://cdn.discordapp.com/attachments/915656975696687124/916022894918795304/12-Month_Subscriber.png https://cdn.discordapp.com/attachments/915650094697693184/933641907052118016/Lebb.jpeg LEBB can I put my bawls in yo jaw [/][/] mendoBanger`);
});

COMMANDS.set('dog', (bot, username, msg) => {
  bot.sendChatMsg(`https://c.tenor.com/0BZW0dfnmycAAAAd/dog-sus.gif bigTeaTime`);
});

COMMANDS.set('doordashmcdonaldstolebbsaddress', async (bot, username, msg) => {
  bot.sendChatMsg(`LEBB's order is currently on its way. peepoSitti `);
  await sleep(30000)
  bot.sendChatMsg("peepoSadDankSip LEBB's order was cancelled.")
});

COMMANDS.set('jensai', (bot, username, msg) => {
  bot.sendChatMsg(`guraFlick`);
});

COMMANDS.set('horsen', (bot, username, msg) => {
  bot.sendChatMsg('https://cdn.discordapp.com/attachments/839616231853654016/985140595322413086/monkaE.png monkaE ');
});



COMMANDS.set('brainpower', (bot, username, msg) => {
  bot.sendChatMsg('O-oooooooooo AAAAE-A-A-I-A-U- JO-oooooooooooo AAE-O-A-A-U-U-A- E-eee-ee-eee AAAAE-A-E-I-E-A-JO-ooo-oo-oo-oo EEEEO-A-AAA-AAAA');
});



COMMANDS.set('homies', async (bot, username, msg) => {
bot.sendChatMsg('TriHard [f][b]7 Signal Received')
await sleep(1000)
bot.sendChatMsg('TriHard [b]Cx')
await sleep(50000)
bot.sendChatMsg('IF YOU SEE THIS VI VON ZULUL ')
await sleep(1000) 
bot.sendChatMsg('!poof')
});
...
COMMANDS.set('8ball', (bot, username, msg) => {
  const answers = [
    'It is certain.', 
    'it is decidedly so.',
    'Without a doubt.',
  ...
  const words = msg.split(' ');
  if (!msg) {
    bot.sendChatMsg(`StareDown ${username} you need to ask a qustion.`)
  } else {
    bot.sendChatMsg(`${username}, [b]${answer}`);
  }
});


// come back to it later when I learn more. Too complex for me right now.(need to learn how to see when someone chats and when bot restarts.)
COMMANDS.set(`mail`, async (bot, username, msg) => {
  const target = msg.split(' ')[0]
  const message = msg.split(' ').slice(1).join(' ');
if (!msg) {
  bot.sendChatMsg(`${username} Please specify target. (@mail "name" "message")`);
  bot.sendChatMsg(`[pink] this command is still a work in progress.`)
  return;
} else {
  bot.sendChatMsg(`${target} DinkDonk ${message} `);
  await sleep(50)
  bot.sendChatMsg(`[pink] this command is still a work in progress.`)
}  
});
...
COMMANDS.set('troll', async (bot, username, msg) => {
  const target = msg.split(' ')[0]
  const message = msg.split(' ').slice(1).join(' ');
  if (isTrolling === true) {
    bot.sendChatMsg(`Trolling [pink][b]I am already trolling someone.`)
    return; }
  if (target === 'ItMePeachy') {
    bot.sendChatMsg(`NOPE [pink][b]I can't let you do that ${username}`);
    return;
  }
  if (!msg) {
    bot.sendChatMsg(`${username} Please specify target. (@troll "name" "message")`);
    return;
  } else {
  bot.sendChatMsg(`TrollRun [b][pink]Trolling ${target}`);
//  bot.sendPm(target, `To stop spam put [b]@stoptrolling [/]in chat`)
  isTrolling = true;
  while (isTrolling === true) {
    bot.sendPm(target, `${message}`);
    await sleep(1000);
  }
}
});

COMMANDS.set('stoptrolling', async (bot, username, msg) => {
  isTrolling = false;
  isRngTrolling = false;
  await sleep(50);
  bot.sendChatMsg(`TooMuchTroIIing  [b][pink] Done Trolling.`);
});

COMMANDS.set('xsogon', async (bot, username, msg) => {
  bot.sendChatMsg(`https://cdn.discordapp.com/attachments/986926739190063114/995664017471312013/XsogonMyBeloved.gif `);
});

let isRngTrolling = false;

COMMANDS.set('rngtroll', async (bot, username, msg) => {
  const randomIndex = randomInt(bot.userlist.length - 1);
  const randomUser = bot.userlist[randomIndex];
  if (isRngTrolling === true) {
    bot.sendChatMsg(`Trolling [pink][b]I am already trolling someone.`)
    return; 
  }
  if (randomUser === 'JohnRG123') {
    bot.sendChatMsg(`NOPE [b][pink]I'm not allowed to troll them`);
    return;
  } else { 
  bot.sendChatMsg(`TrollRun [b][pink]Trolling ${randomUser.name}`);
  isRngTrolling = true;
  while (isRngTrolling === true) {
    bot.sendPm(randomUser.name, `DogLookingSussyAndHappy [pink] to stop spam put [b]@stoptroll [/]in chat.`);
    await sleep(1000);
  }
}
});

COMMANDS.set('stoptroll', async (bot, username, msg) => {
  bot.sendChatMsg(`TooMuchTroIIing  [b][pink] Done Trolling.`);
  await sleep(50);
  isTrolling = false;
  return isRngTrolling = false;
});




// to allow me to talk with bot easier to use than @spam 1 (message)
COMMANDS.set('say', async (bot, username, msg) => {
  if ((!(await bot.checkPermission(username, Rank.OWNER, null))) && (username !== 'ItMePeachy')) {
    bot.sendChatMsg(`Awkward [pink][b]${username}[/][pink] you're not allowed to do use that`);
    return;
  }
  if (!msg) {
    bot.sendPm(username, `[pink]Please tell me what to say. [b]@say (message)`);
    return;
  } else {
    bot.sendChatMsg(`${msg}`);
  }
...
COMMANDS.set('peachy', async (bot, username, msg) => {
bot.sendChatMsg('⠀');
await sleep(150);
bot.sendChatMsg(`https://cdn.discordapp.com/attachments/904501042148958219/981805543121371136/ezgif-2-b39e272ae5.png https://cdn.discordapp.com/attachments/962020977808662539/969019181456572506/ezgif-5-72a8fa7b74.gif [pink]ItMePeachy guraClueless `);
});

COMMANDS.set('kit', async (bot, username, msg) => {
bot.sendChatMsg('Кіт ти маму мав? https://cdn.betterttv.net/emote/62b2fbdf65092c1291b968fa/1x.gif Мав');
await sleep(2000);
bot.sendChatMsg('А тату мав? https://cdn.betterttv.net/emote/62b2fbdf65092c1291b968fa/1x.gif Мав');
await sleep(2000);
bot.sendChatMsg('А брата мав? https://cdn.betterttv.net/emote/62b2fbdf65092c1291b968fa/1x.gif Мав');
});


COMMANDS.set('poro', async (bot, username, msg) => {
  bot.sendChatMsg(`PoroSad [b][pink]nonono don hit the poro [/][/] PoroHappy [b][pink]i lov poro [/][/] poroLaugh [b][pink]poro is love poro is life [/][/] elisPoro [b][pink]poro is the best [/][/] poroPls [b][pink]poro poro [/][/] PoroHappy  `);
  });


COMMANDS.set('rngip', async (bot, username, msg) => {
  const ips = [
    '165.145.223.229', 
    '16.201.69.181', 
    '86.31.227.116',
  ...
  const randomIndex = randomInt(bot.userlist.length - 1);
  const randomUser = bot.userlist[randomIndex]; 
  bot.sendChatMsg(`xqcL ${randomUser.name} ${ip}`);
  }
);



COMMANDS.set('fake', async (bot, username, msg) => {
  bot.sendChatMsg(`Fake aaaaaaaaaaaand... Yeah, that's pretty fake. That's fake and- yeah that's That's fake. fake. That's fake. That's fake- That's pretty fake. That's pretty fake, I mean-...  Yeah. That's fake. Uhm- That's fake. That's firetruckin' fake. Uhm... That's how it is dude. PeepoGladDank `);
});


COMMANDS.set('rngssn', async (bot, username, msg) => {
    const ssns = [
      '483-54-5870', 
      '035-58-2612', 
      '238-11-1121',
    ...
    const ssn = ssns[randomInt(ssns.length)];
    const randomIndex = randomInt(bot.userlist.length - 1);
    const randomUser = bot.userlist[randomIndex];
    bot.sendChatMsg(`PoroHappy [b][pink]${randomUser.name}'s Social Security number is ${ssn}`)
});



COMMANDS.set('ip', async (bot, username, msg) => {
  const target = msg.split(' ')[0]
  const ips = [
    '165.145.223.229', 
    '16.201.69.181', 
  ...
  const ip = ips[randomInt(ips.length)];
  const randomIndex = randomInt(bot.userlist.length - 1);
  bot.sendChatMsg(`PoroHappy [b][pink]${target}'s ip is ${ip}`);
});

COMMANDS.set('ssn', async (bot, username, msg) => {
  const target = msg.split(' ')[0]
  const ssns = [
    '483-54-5870', 
    '035-58-2612', 
  ...
  const ssn = ssns[randomInt(ssns.length)];
  bot.sendChatMsg(`PoroHappy [b][pink]${target}'s Social Security number is ${ssn}`)
});
  

COMMANDS.set('dinner', (bot, username, msg) => {
  bot.sendChatMsg('LETTUCE PANCAKE Melon CORN Cucumber STEAK ICECREAM potassium COB @cookie');
});



COMMANDS.set('ping', async (bot, username, msg) => {
  bot.sendChatMsg(`click out of the browser [b]${username}`)
  await sleep(2500)
  bot.sendChatMsg(`DinkDonk [b]${username} `);
});

COMMANDS.set('flipacoin', (bot, username, msg) => {
  const coins = [
    'Heads', 
    'Tails',
  ];
  const coin = coins[randomInt(coins.length)];
    bot.sendChatMsg(`${username}, it's [b]${coin}[/].`);
});


COMMANDS.set('afk', async (bot, username, msg) => {
  bot.sendChatMsg(`/afk`);
})

COMMANDS.set(`fuzzybrain`, async (bot, username, msg) => {
  if (username != 'ItMePeachy') {
    bot.sendChatMsg(`ERM you're not peachy`);
    return;
  }
  let pos = 'next'; 
  const vid = parseMediaLink(`https://youtube.com/playlist?list=PLHEKy_CoXxKhNw9_wnrmgPEltBHcOuJCr`);
      bot.addVideo(null, null, null, pos, vid);
});

COMMANDS.set('wolfram', async (bot, username, msg) => {
  bot.sendChatMsg(`@wolfram was replaced with [b]@lookup`);
})
