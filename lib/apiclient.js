...
/**
 * Metadata about a Dad Joke.
 *
 * @typedef {Object} DadJoke
 * @property {!Object} body The body containing the response.
 *
 */

/**
 * Makes an API call do rapidapi.com for dad joke
 *
 * @return {!Promise<DadJoke>} dad joke.
 */
export async function callDadJoke() {
  const options = {
    ...
    headers: {
      'X-RapidAPI-Key': '9ad975b2b8msh427a4df7ab467a6p1ce6eajsn69b6a6293368',
  ...
  axios.request(options)
      .then(function(response) {
        infoLog.log(response.data);
      })
      .catch(function(error) {
        console.error(error);
      });
  infoLog.log(`Type: ${response.data.entries}`);
  return {
...
/**
 * Makes an API call to YouTubeConverter.
 *
 * @param {string} url The URL for the youtube video to convert.
 * @return {!Promise<string>} URL of download.
 */
export async function callYouTubeConverter(url) {
  const options = {
    method: 'GET',
...
/**
 * API call for YouTube videos.
 * Used to search for a video by name.
 *
 * @param {string} search String to search for video.
 * @param {string} apiKey YouTube API key.
 * @return {!Promise<YouTubeVideoMeta>} Metadata about the video.
 */
export async function youTubeSearch(search, apiKey) {
  ...
  return {
    id: resp.data.items[0].id.videoId,
