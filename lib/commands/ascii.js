...
class Limiters {
  ...
  static GAYGE = new RateLimiter({tokensPerInterval: 1, interval: 180 * 1_000});
}
...
COMMANDS.set('gayge', async (bot, username, msg) => {
  const price = 0;
  const currentPoints = await bot.db.getUserPoints(username);
  if (currentPoints < price) {
    bot.sendPm(username, `You don't have enough points for this command(${price})`);
    ...
    if (!Limiters.GAYGE.tryRemoveTokens(1)) {
      return bot.sendPm(username, '$gayge is on cooldown.');
    } else {
      await bot.db.updateUserPoints(username, -price);
    }
  }

  await bot.setExCSS(`https://ip0g.github.io/CyDJcss/colorshift.css`);
  ...
  await sleep(30 * 1000);
  await bot.setExCSS(`https://ip0g.github.io/CyDJcss/main.css`);
});
