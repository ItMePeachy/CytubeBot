...
/**
 * A user on the time leaderboard.
 *
 * @typedef {Object} TimeLeaderboardUser
 * @property {string} name Name of the user on the leaderboard.
 * @property {string} time Number of time that user has.
 */


/**
 * A user on the cookies leaderboard.
 *
 * @typedef {Object} CookiesLeaderboardUser
 * @property {string} name Name of the user on the leaderboard.
 * @property {string} cookies Number of cookies that user has.
 */

/**
 * A user on the dabloons leaderboard.
 *
 * @typedef {Object} DabloonsLeaderboardUser
 * @property {string} name Name of the user on the leaderboard.
 * @property {string} dabloons Number of dabloons that user has.
 */
...
export class Database {
  ...
  createTables() {
    ...
    this.db.run(
        'CREATE TABLE IF NOT EXISTS ' +
        'dabloons(user TEXT, count INTEGER, last_claim_timestamp INTEGER, PRIMARY KEY(user))');
  }
  ...
  async updateTables() {
    ...
    await this.updateDbVersion(3, async () => {
      await dbRun(this.db, 'ALTER TABLE users ADD time INTEGER DEFAULT 0');
    });
   ...
   * @param {string} type The video type eg. "yt".
   * @param {string} id The ID of the video.
   * @param {number} unflag The flag, should be 0
   * @param {string} title Title of the video.
   */
    unflagVideo(type, id, unflag, title) {
      auditLog.log(`*** unflagging video: ${title} with flag: ${unflag}`);
  
      const statement =
          this.db.prepare('UPDATE videos SET flags = ? WHERE type = ? AND id = ?', [unflag, type, id]);
      statement.run();
  ...
  /**
   * Update a user's time.
   *
   * @param {string} username The user whose Time we are changing.
   * @param {number} delta Change in the user's Time to apply, i.e. 5 or -10.
   * @return {!Promise<void>} Promise indicating the change's completion.
   */
  async updateUserTime(username, delta) {
    const currentTime = await this.getUserTime(username);
    const newTime = (currentTime + delta) >= 0 ? (currentTime + delta) : 0;
    const statement = this.db.prepare(
        'UPDATE users SET time = ? WHERE LOWER(uname) = ?', [newTime, username.toLowerCase()]);
    return dbRunStatement(statement);
  }
  ...
  /**
   * Updates dabloon count for a user.
   *
   * Increments count by n and sets last_claim_timestamp to now.
   *
   * @param {string} user The user to tempban.
   ...
   * @return {!Promise<void>} Empty promise indicating the operation's completion.
   */
    async updateUserDabloon(user, n) {
      await dbRunStatement(this.db.prepare(
          'INSERT OR IGNORE INTO dabloons(user, count, last_claim_timestamp) VALUES (?, ?, ?)',
          [user, 0, 0]));
  
      const count = await this.getUserDabloonCount(user);
  
      const update = this.db.prepare(
          'UPDATE dabloons SET count = ?, last_claim_timestamp = ? WHERE user = ?',
          [count + n, DateTime.now().toUnixInteger(), user]);
    return dbRunStatement(update);
  }
  ...
  /**
   * Updates cookie count for a user getting or recieving cookies.
   *
   * Increments count by n and sets last_claim_timestamp to now.
   *
   * @param {string} user The user to tempban.
   * @param {number} n Number of cookies to add or remove.
   * @return {!Promise<void>} Empty promise indicating the operation's completion.
   */
  async giveUserCookie(user, n) {
    await dbRunStatement(
        this.db.prepare('INSERT OR IGNORE INTO cookies(user, count) VALUES (?, ?)', [user, 0]));

    const update = this.db.prepare('UPDATE cookies SET count = ? WHERE user = ?', [n, user]);
    return dbRunStatement(update);
  }


    /**
   * Updates dabloon count for a user getting or recieving dabloons.
   *
   * Increments count by n and sets last_claim_timestamp to now.
   *
   * @param {string} user The user to tempban.
   * @param {number} n Number of dabloons to add or remove.
   * @return {!Promise<void>} Empty promise indicating the operation's completion.
   */
    async giveUserDabloon(user, n) {
      await dbRunStatement(
          this.db.prepare('INSERT OR IGNORE INTO dabloons(user, count) VALUES (?, ?)', [user, 0]));
  
      const update = this.db.prepare('UPDATE dabloons SET count = ? WHERE user = ?', [n, user]);
      return dbRunStatement(update);
    }
  ...
  /**
   * Get phrase count - number of times a phrase has been used.
   *
   * @param {string} phrase Phrase to get the count for.
   *
   * @return {!Promise<number>} The number of phrases sent.
   */
  ...
  /**
   * Gets a page of cookie leaderboard data.
   *
   *
   * @param {number} pageNumber Gets a given leaderboard page.
   * @param {number} pageSize Number of items to return in the page.
   * @return {!Promise<!Array<!CookiesLeaderboardUser>>} The leaderboard data.
   */
  async getCookieLeaderboardPage(pageNumber, pageSize) {
    const query = `
          SELECT user AS name,
                  count
          FROM cookies
    ...
    /**
   * Gets a page of dabloon leaderboard data.
   *
   *
   * @param {number} pageNumber Gets a given leaderboard page.
   * @param {number} pageSize Number of items to return in the page.
   * @return {!Promise<!Array<!DabloonsLeaderboardUser>>} The leaderboard data.
   */
    async getDabloonLeaderboardPage(pageNumber, pageSize) {
      const query = `
            SELECT user AS name,
                    count
            FROM dabloons
  ...
  async getPointsLeaderboardPage(pageNumber, pageSize) {
    ...
    return new Promise((resolve, reject) => {
      ...
      /**
   * Gets a page of time leaderboard data.
   *
   * @param {number} pageNumber Gets a given leaderboard page.
   * @param {number} pageSize Number of items to return in the page.
   * @return {!Promise<!Array<!TimeLeaderboardUser>>} The leaderboard data.
   */
      async getTimeLeaderboardPage(pageNumber, pageSize) {
        const query = `
              SELECT uname AS name,
                      time
              FROM users
  ...
  /**
   * Gets the current user's dabloon count.
   *
   * @param {string} user The user we are looking up.
   * @return {!Promise<number>} The user's current number of dabloons.
   */
  async getUserDabloonCount(user) {
    await dbRunStatement(this.db.prepare(
        'INSERT OR IGNORE INTO dabloons(user, count, last_claim_timestamp) VALUES (?, ?, ?)',
        [user, 0, 0]));

    const statement = this.db.prepare('SELECT count FROM dabloons WHERE user = ?', [user]);
  ...
  async getUserCookieLastClaimTimestamp(user) {
    ...
    /**
   * Gets the time at which the user last claimed a dabloon.
   *
   * @param {string} user The user we are looking up.
   * @return {!Promise<number|null>} Unix timestamp of the last claim time, or null if the user has
   *     never claimed a dabloon.
   */
    async getUserDabloonLastClaimTimestamp(user) {
      await dbRunStatement(this.db.prepare(
          'INSERT OR IGNORE INTO dabloons(user, count, last_claim_timestamp) VALUES (?, ?, ?)',
          [user, 0, 0]));
  
      const statement =
          this.db.prepare('SELECT last_claim_timestamp FROM dabloons WHERE user = ?', [user]);
  ...
  *
  * @param {string} username User to look up.
  * @return {!Promise<number>} The user's time.
  */
 getUserTime(username) {
   const statement = this.db.prepare(
       'SELECT time FROM users WHERE LOWER(uname) = ?', [username.toLowerCase()]);
