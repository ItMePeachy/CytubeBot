...
COMMANDS.set('addpoints', async (bot, username, msg) => {
  ...
  if (!isNaN(parseInt(user, 10))) {
    bot.sendChatMsg(
        ...
        'Example: @addpoints ItMePeachy 100000 PagMan');
    return;
  }
  ...
  if (msg.split(' ')[1] === undefined) {
    bot.sendChatMsg(
        ...
        'Example: @addpoints ItMePeachy 100');
    return;
  }
...
COMMANDS.set('givepoints', async (bot, username, msg) => {
  ...
  if (msg.split(' ')[1] === undefined) {
    bot.sendChatMsg(
        'Points amount must be provided. ' +
        'Example: @givepoints ItMePeachy 100');
    return;
  }
  ...
  if (isNaN(givingAmount)) {
    bot.sendChatMsg('Failed to parse points amount. Example: @givepoints ItMePeachy 100');
    return;
  }
...
COMMANDS.set('timeleaderboard', async (bot, username, msg) => {
  const pageSize = 5;
  const pageMsg = msg.split(' ')[0];
  const pageMsgParsed = pageMsg !== '' ? parseInt(pageMsg, 10) : 1;
  const pageNumber = !isNaN(pageMsgParsed) ? pageMsgParsed : 1;
  ...
  const page = await bot.db.getTimeLeaderboardPage(pageNumber - 1, pageSize);
  const start = firstRank;
  const end = firstRank + page.length - 1;
  ...
  deliver(`Time leaderboard ${start}-${end}:`);

  for (const [i, user] of page.entries()) {
    const rank = i + firstRank;
    deliver(`#${rank} ${user.time} minutes: ${user.name}`);
  }
...
// eslint-disable-next-line valid-jsdoc
const /** @type {Handler} */ timeHandler = async (bot, username, msg) => {
  const target = msg.split(' ')[0] !== '' ? msg.split(' ')[0] : username;
  ...
  const pageSize = 100;
  pageLoop: while (true) {
    const page = await bot.db.getTimeLeaderboardPage(pageNumber, pageSize);
    for (const user of page) {
      if (user.name === target) {
        time = user.time;
        break pageLoop;
  ...
  bot.sendChatMsg(`[pink]${target} has been active in cydj for [b]${time} minutes[b][/] (rank ${rank})`);
};

COMMANDS.set('time', timeHandler);
...
COMMANDS.set('mraffle', async (bot, username, msg) => {
  if (username != 'ItMePeachy') {
    bot.sendChatMsg(`${username} RAGEY youre not allowed to use mraffle cheater!`);
    return;
  }
  if (bot.gamba.raffleInProgress) {
    bot.sendChatMsg('A raffle is already in progress Pepega');
  ...
  let /** @type {number} */ points;
  if (pointsMsg.endsWith('k')) {
    points = (parseInt(pointsMsg, 10)) * 1000;
  } else {
    points = parseInt(pointsMsg, 10);
  }
  if (isNaN(points)) {
    bot.sendChatMsg(`Failed to parse points amount. ex: $mraffle 1000 / $mraffle 1k`);
    return;
  }
  if (points < 0) {
    bot.sendChatMsg(`Amount must not be less than 0 Pepega`);
    return;
  }
  if (points > maxAmount) {
    bot.sendChatMsg(`Amount ${points - maxAmount} too large`);
    return;
  }
...
COMMANDS.set('removepoints', async (bot, username, msg) => {
  ...
  if (!isNaN(parseInt(user, 10))) {
    bot.sendChatMsg(
        'Username must be provided for removepoints. Example: @removepoints ItMePeachy 100000 :tf:');
    return;
  }

  if (msg.split(' ')[1] === undefined) {
    bot.sendChatMsg('Points amount must be provided. Example: @removepoints ItMePeachy 100');
    return;
  }
...
COMMANDS.set('timerank', async (bot, username, msg) => {
  const target = msg.split(' ')[0] !== '' ? msg.split(' ')[0] : username;
  ...
  bot.sendChatMsg(`${target} is rank ${rank} on the leaderboard with ${time} minutes active in cydj`);
});
