...
import {callAnagram, callDadJoke, callWolfram, WEATHER_ABBREVIATION, weatherFromLocation, weatherFromZipCode} from '../apiclient.js';
...
COMMANDS.set('dadjoke', async (bot, username, msg) => {
  if (!(await bot.checkPermission(username, Rank.MOD))) {
    bot.sendChatMsg(`${username} doesn't have permission FeelsWeirdMan`);
    return;
  }
  let /** @type {DadJoke} */ joke;
  try {
    joke = await callDadJoke();
  } catch (e) {
    errorLog.log(`Failed to get a dad joke: ${e}`);
    bot.sendChatMsg('Failed to get a dad joke');
    return;
  }
  infoLog.log(`# ${joke.body}`);
  // infoLog.log(typeof joke.body);
  // bot.sendChatMsg(`${JSON.stringify(joke.setup)}`);
  await sleep(5000);
  // bot.sendChatMsg(`${joke.punchline}`);
});
...
COMMANDS.set('userlogs', async (bot, username, msg) => {
  const channel = msg.split(' ')[0] !== '' ? msg.split(' ')[0] : 'xqc';
  if (msg.split(' ')[1] == null) {
...
COMMANDS.set('lookup', async (bot, username, msg) => {
