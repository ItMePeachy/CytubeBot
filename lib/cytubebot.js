...
import {Dabloons} from './dabloon.js';
...
export class CytubeBot {
  ...
  async init(config) {
    ...
    /** @type {RateLimiter} */
    this.lastseenLimiter = new RateLimiter({tokensPerInterval: 2, interval: 15 * 1_000});
    /** @type {number} */
    ...
    this.dabloons = new Dabloons(this.db);
    this.gamba = new Gamba();
  ...
  blacklistVideo(uid) {
    ...
    /**
   * Unblacklists the current video or uid.
   *
   * Used by $unblacklist.
   *
   * @param {string} uid A video we want to unblacklist.
   */
  unblacklistVideo(uid) {
    const flag2 = 0;
    let {type, id, title} = this.currentMedia;
    ...
    this.db.unflagVideo(type, id, flag2, title);
  }
  ...
  grantAutomaticUserPoints(activePoints = 0, afkPoints = 0) {
    ...
    /**
   * Time tracker spent in cydj
   * Grants "automatic" time to users currently in the room.
   *
   * @param {number} activeTime Number of time to grant to active users.
   * @param {number} afkTime Number of time to grant to AFK users.
   */
    grantAutomaticUserTime(activeTime = 0, afkTime = 0) {  
      for (const user of this.userlist) {
        if (user.name === this.username) {
          continue;
        }
  
        const time = user.meta.afk ? afkTime : activeTime;
        this.db.updateUserTime(user.name, time);
      }
    }
  ...
  async handleChatMsg(data, isPm) {
    ...
    if (username === '[server]') {
      const uName = msg.split(' ')[0];
      const joinMsg = msg.split(' ')[1];
        return;
    }
    ...
    for (let w = 0; w < words.length; w++) {
      if (username != 'PeachyBot' && words[w].startsWith('PeachyBot')) {
        const ping = randomInt(100) > (98);
        if (ping === true) {
          this.sendChatMsg(`muniREE [b][pink]${username} STOP PINGING ME!`);
        }
      }
    }
    ...
    for (let w = 0; w < word.length; w++) {
      if (username != 'PeachyBot' && word[w].startsWith('juicer')) {
        const juice = randomInt(100) > (95);
        if (juice === true) {
          this.sendChatMsg(`ICANTSTOPfiretruckINGBORTING I FREAKING LOVE XQC HES SO FUNNY AND RICH AND I LOVE GIVING MORE MONEY TO A MULTI MILLIONAIRE SO I CAN HAVE THE SLIGHTEST CHANCE OF HIM SAYING MY NAME BECAUSE HES MY BESTEST FRIEND`);
        }
      }
    }

    if (msg.includes('pechy')) {
    this.sendChatMsg('⠀');
    this.sendChatMsg('https://cdn.discordapp.com/attachments/986926739190063114/1037712558183960716/NoOneIsAroundToHelp.gif https://cdn.discordapp.com/attachments/986926739190063114/1037701659494723636/Cat.gif [pink]ItMePeachy[/] nekoPout not my name');
    }

       
    if (msg.includes('whats the script')) {
      this.sendChatMsg('[b]the script to watch google drive videos:[/] https://cytu.be/google_drive_userscript');
      }

    if (msg.includes('ItMePeachy claimed')) {
      const peachyy = 'ItMePeachy'
      await sleep (7200000)
      this.sendPm(peachyy, 'MenheraDoorBell [b][pink]you can get your cookie again!');
      }
    
      if ((msg.includes('%AM%')) && (username != this.username)) {
        const pingableMods = this.userlist.filter((user) => user.name !== this.username)
                                 .filter((user) => user.rank >= Rank.MOD)
                                 .map((user) => user.name);
        if (!(pingableMods.includes(username))) {
          const timeout = msg.split('%');
          const lengthSecs = parseInt(timeout[2]);
          const end = getCurrentUnixTimestamp() + lengthSecs;
          const lengthDesc = humanizeDuration(lengthSecs * 1000);
          this.sendPm(
              username,
              `[green]AutoMod:[/] ${username}: a ${lengthDesc} timeout ` +
                  `[red]Msg:[/] ${msg}`);
          for (let i = 0; i < pingableMods.length; i++) {
            this.sendPm(
                pingableMods[i],
                `[green]AutoMod:[/] ${username}:` +
                    ` ${lengthDesc} [red]Msg:[/] ${msg}`);
          }
          this.sendChatMsg(`/mute ${username}`);
          this.db.setUserTimeout(username, end);
        }
      }

    if ((msg.includes('%AMS%')) && (username != this.username)) {
      const Mods = this.userlist.filter((user) => user.name !== this.username)
                       .filter((user) => user.rank >= Rank.MOD)
                       .map((user) => user.name);
      if (!(Mods.includes(username))) {
        ...
        const end = getCurrentUnixTimestamp() + lengthSecs;
        const lengthDesc = humanizeDuration(lengthSecs * 1000);
        this.sendChatMsg(`/me Timed out ${username} for ${lengthDesc}`);
        this.sendChatMsg(`/mute ${username}`);
        this.db.setUserTimeout(username, end);
      }
    }
    ...
    if (handleCommand) {
      handleChatMessage(this, username, msg, isPm);
      return;
    }
   ...
   * Sets Channel CSS
   *
   * @param {string} CSS The new CSS to update
   *
   */
  async setCSS(CSS) {
    this.socket.emit('setChannelCSS', {
      css: CSS,
    });
  }

  /**
   * Handles Channel CSS Update
   *
   * @param {?} data Channel CSS data
   *
   */
  async handleChannelCSSUpdate(data) {
    infoLog.log(`!%%%%! Channel CSS updated`);
  }

  /**
   * Reads a CSS string from a txt file
   *
   * @return {string} The CSS as a string to send as socket msg
   *
   */
  async readTestCSS() {
    try {
      return await readFile('./testcssLen.txt', 'utf-8');
    } catch (err) {
      this.sendChatMsg(`ERROR readTestCSS(): ${err}`);
    }
  }
  ...
  async writeCSS(css) {
    try {
      writeFile('./testcss2.txt', `${css}`);
    } catch (err) {
      this.sendChatMsg(`Error writing css: ${err}`);
    }
  }
   ...
   * Sets external css
   *
   * @param {string} link The external css link
   *
   */
  async setExCSS(link) {
  ...
  async handleTimersPostRestart() {
    try {
      ...
      for (let i = 0; i < tEnd.length; i++) {
        if ((getCurrentUnixTimestamp() > tEnd[i]) && (tEnd[i] !== 0) && (tPhr !== null)) {
          this.sendChatMsg(
  ...
  async handleLogin(data) {
    ...
    this.sendChatMsg(
        //'linaxWave [b][pink] PeachyBot at your service!',
        /** override=*/ true);
  ...
  start() {
    ...
    const automaticTimeIntervalMins = 1;
    const automaticActiveTime = 1;
    const automaticAfkTime = 0;
    ...
    setInterval(
        () => this.grantAutomaticUserTime(automaticActiveTime, automaticAfkTime),
        automaticTimeIntervalMins * 60 * 1000);
   ...
   */
  startAutoQueueing() {
    const timeBeforeQueueing = Duration.fromObject({minutes: 5});
